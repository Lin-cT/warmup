# Makefile for the Asian-United-Frontend repository

# Configuration, override port with usage: make PORT=4300
PORT ?= 9008
REPO_NAME ?= Asian-United-Frontend
...
default: server
	...
	@# Start an infinite loop with a timeout to check log status
	...
	@# outputs startup log, removes the last line ($$d) as the ctl-c message is not applicable for a background process
	@sed '$$d' $(LOG_FILE)
...
# Clean up project-derived files, to avoid run issues, stop is a dependency
clean: stop
...
stop:
	...
	@# kills the process running on port $(PORT)
